defmodule Sci.Signal.TransformTest do
  ...
  describe "bilinear/4" do
    ...
    test "shifts poles" do
      expected_poles = [~n{-0.045436579714941916+0.6672218723563778i}, ~n{-0.03266828910227484+0.1987100804126568i}, ~n{-0.03266828910227484-0.1987100804126568i}, ~n{-0.045436579714941916-0.6672218723563778i}]
      provided_poles = [~n{-0.40752+0.98383i}, ~n{-0.98383+0.40752i}, ~n{-0.98383-0.40752i}, ~n{-0.40752-0.98383i}]
    ...
    test "transforms gain to compensate for shifted zeros, poles" do
      ...
      assert {_, _, 0.10630859774129929} = bilinear([], [~n{-0.40752+0.98383i}, ~n{-0.98383+0.40752i}, ~n{-0.98383-0.40752i}, ~n{-0.40752-0.98383i}], 1.2859, 2.0)
